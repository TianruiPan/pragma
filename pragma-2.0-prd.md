...
> 跨仓契约：`pragma-integration/v2`；Governance Design Gate/finalizer 是 Design Issue 完成回填与控制状态的唯一 writer
...
12. 设计 PR 合入默认分支后，Governance Design Gate/finalizer 从 canonical current/manifest 和 merged PR 推导事实，通过 controlled writer 回填 Design Issue 的 current pointer、manifest、版本、package checksum、PR 和 merged commit。
...
```json
{
  ...
  "artifact": {
    ...
    "fileName": null,
    "checksum": "sha256:...",
    "path": ".pragma/design-contexts/issue-123/versions/v1"
  }
}
```

`manifest.packageChecksum` 表示 canonical package payload identity。计算时排除 `manifest.json`、`checksums.json`、`context.zip`、`handoff/issue-fragment.md` 和 `handoff/pipeline-summary.json`，避免 checksum 自引用，也不让 storage serialization 改变 package identity。`manifest.artifact.checksum` 表示存储字节完整性：repo storage 时等于 package checksum；MinIO storage 时是 `context.zip` 的精确 SHA-256，可以与 package checksum 不同。

MinIO archive 只包含 payload，不包含 `manifest.json` / `checksums.json` 控制文件。Runner 始终以 pinned 默认分支上的 manifest 为唯一权威，descriptor 的 `checksum` 表示 package checksum，MinIO metadata/download/cache key 使用 artifact checksum。旧 `pragma-integration/v1` 包不得被静默重解释，必须重新发布为 v2。
...
```text
PRAGMA_CONTEXT_MINIO_ENDPOINT=http://218.11.1.13:9000
PRAGMA_CONTEXT_MINIO_BUCKET=product-project-dev-lab
PRAGMA_CONTEXT_MINIO_REGION=us-east-1
PRAGMA_CONTEXT_MINIO_OBJECT_PREFIX=pragma-design-context
PRAGMA_CONTEXT_MINIO_ACCESS_KEY=<secret-manager / environment>
PRAGMA_CONTEXT_MINIO_SECRET_KEY=<secret-manager / environment>
```

MVP 按 ADR-008 使用一套共享 `PRAGMA_CONTEXT_MINIO_*` key pair：Pragma publisher / Capture Bridge 可在批准 prefix 内发布，Governance Design Gate 只走 metadata 路径，Runner materializer 只走 read 路径。该身份必须限制在批准 bucket 与 `pragma-design-context/` prefix；Access Key、Secret Key、session token 或 presigned URL 均不得写入 PRD、manifest、Issue、日志或 Git，也不得进入 Governance API、Codex app-server、Agent shell、repo hooks 或 prompt。当前 endpoint 是 HTTP，只允许通过受控公司网络访问；生产优先启用 TLS 或受信代理终止 TLS。
...
开发消费不依赖 Pragma CLI。Governance Runner 在 app-server turn 启动前按 `pragma-integration/v2` 原生解析 package，并输出：
...
descriptor 的 `checksum` 是 package checksum。Runner 对 repo-native 目录和 materialized MinIO payload 都必须独立重算 canonical package checksum。当 `manifest.artifact.storage` 是 `minio-s3` 时，Runner 的可信 pre-dispatch materializer 使用共享 MVP 身份的 read 路径下载。artifact 写入 Git workspace 之外、按 artifact checksum 寻址的 cache；完成 bucket allowlist、确定性 objectKey、压缩/解压大小、文件数量、archive-byte checksum、canonical payload checksum、绝对路径、`../`、symlink/hardlink 校验后，才以只读路径交给 Agent。MinIO Access/Secret Key 不进入 app-server、Agent shell、hooks、prompt 或开发者电脑。
...
-> Governance Design Gate/finalizer 通过 controlled writer 回填 Design Issue
...
生成供人类诊断/复制的 Design Issue markdown 事实，包含 current pointer、manifest、package path / URL、version、package checksum、context PR、merged commit 占位；它不是正常 Governance wiring。实际回填由 Design Gate/finalizer 从默认分支与 merged PR 推导并通过 controlled writer 完成，评论、状态流转和 dependency 创建也不由 Pragma 负责。
...
8. 设计 PR 合入后，Governance Design Gate/finalizer 从 canonical package facts 与 merged PR 回填 Design Issue #102。
```
...
5. Runner 读取 current.json / manifest，pin source commit、version 和 package checksum
6. MinIO 大包由可信 materializer 写入 artifact-checksum-keyed cache，并只读挂载
...
14. 实现代码并在 PR 记录 Design Issue、source commit、version、manifest path 和 package checksum
```
...
- 如存在 `context.zip`，其精确字节 SHA-256 与 `manifest.artifact.checksum` 一致；不得要求它等于 `manifest.packageChecksum`；
...
- 如 context 超过 20MB，MinIO bucket/objectKey 可由 metadata/read 路径访问，metadata 与 archive bytes 均匹配 artifact checksum。
...
5. Design Issue 可以由 Governance Design Gate/finalizer 从“待生成”回填 current pointer、Manifest、Package、版本、package checksum、PR 和 merged commit；写入通过 Permission Service、verified control state 与 outbox 完成。
...
- Governance Design Gate/finalizer 从默认分支 canonical current/manifest 与 merged Context PR 回填 Design Issue；`issue-fragment` 只保留为人类诊断输出；
- Governance Runner 能在没有 Pragma CLI 时根据 Design Issue 原生解析 current.json 和 manifest，并输出 `pragma-context-descriptor/v1`；
- MinIO 大包通过隔离的 pre-dispatch materializer 写入 artifact-checksum-keyed cache，并只读交付给 Agent；
...
| MinIO 下载/解压不安全 | shared credential 泄漏、路径穿越、缓存污染或压缩炸弹 | bucket/prefix allowlist、隔离 read 路径、artifact-checksum-keyed cache、archive 安全上限、只读挂载 |
...
Design PR 合入默认分支后，Governance Design Gate/finalizer 通过 controlled writer 回填 Pragma current pointer / manifest / package
